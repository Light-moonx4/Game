...
def turn_enemy(h_hp, e_hp):
    # Primero la condición más crítica (autodestrucción)
    if e_hp <= 5:
        dmg_e = 50 # Daño fijo o damage_generator(50, 60)
        h_hp -= dmg_e
        e_hp = 0   # El enemigo muere al explotar
        print(f"\nEnemy SELF-DESTRUCTS! Massive damage: {dmg_e}")
    # Luego la condición de ataque fuerte
    elif e_hp < 30:
        dmg_e = damage_generator(20, 25)
        h_hp -= dmg_e
        print(f"\nEnemy is desperate! Strong attack: {dmg_e}")
    # Ataque normal
    else:
        dmg_e = damage_generator(15, 20)
        h_hp -= dmg_e
        print(f"\nEnemy attacks! Damage: {dmg_e}")
